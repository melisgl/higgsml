  ...
  (let ((*default-mat-ctype* :float))
    ...
    (destructuring-bind (training-file trained-model-dir) args
      (let ((*training-file* training-file)
            (*test-file* nil)
            (trained-model-dir
              (osicat:pathname-as-directory trained-model-dir)))
...
(defun higgsml-predict (args)
  (let ((*default-mat-ctype* :float))
    ...
    (destructuring-bind (test-file trained-model-dir submission-file) args
      (let* ((trained-model-dir
               (osicat:pathname-as-directory trained-model-dir))
             (*training-file*
               (merge-pathnames "training.csv" trained-model-dir))
             (*test-file* nil))
...
(defun train* (training-examples trained-model-dir)
  ;; Train a backprop network 10 times on 2 fold cross-validation
...
(defun check-cuda ()
  (if (cuda-available-p)
      (log-msg "CUDA found.~%")
      (log-msg "CUDA not found. Will proceed using BLAS.~%")))
